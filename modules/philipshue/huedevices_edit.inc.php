...
*/
  ...
  if ($this->tab=='data') {
   ...
   if ($this->mode=='update') {
    global $title_new;
	if ($title_new) {
	 $prop=array('TITLE'=>$title_new,'HUEDEVICES_ID'=>$rec['ID']);
	 $new_id=SQLInsert('huedevices_property',$prop);
	}
   }
   global $delete_id;
   if ($delete_id) {
    SQLExec("DELETE FROM huedevices_property WHERE ID='".(int)$delete_id."'");
   }
   $properties=SQLSelect("SELECT * FROM huedevices_property WHERE HUEDEVICES_ID='".$rec['ID']."' ORDER BY ID");
   ...
   for($i=0;$i<$total;$i++) {
    ...
    if ($this->mode=='update') {
      ...
      SQLUpdate('huedevices_property', $properties[$i]);
