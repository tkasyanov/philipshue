...
class philipshue extends module
{
    ...
    function usual(&$out)
    {
        if ($this->ajax) {
            global $op;
            global $uuid;
            if ($op == 'auth' and strlen($uuid)>10 ) {
                $this->getConfig();
                $dev_rec = SQLSelectOne("SELECT * FROM huedevices WHERE  UUID='" . $uuid . "' ");
                if ($dev_rec['ID']) {
                    $maxTries = 30;
                    for ($i = 1; $i <= $maxTries; ++$i) {
                        try {
                            ...
                            $response_text = @file('http://' . $dev_rec["IP"] . '/api/', false, stream_context_create($context))[0];
                            ...
                            if ($response->error) {
                                ...
                                $dev_rec['PASSWORD'] = $response->success->username;
                ...
                echo "\n\n", "Failure to create user. Please try again! error: " ;
            }
        }
        $this->admin($out);
    }
...
LEFT JOIN huedevices ON `HUEDEVICES_ID`=huedevices.ID WHERE  $table.ID=" . (int)$properties[$i]["ID"]);
...

    {
        ...
        $this->getConfig();
        $bridge = SQLSelect("SELECT * FROM huedevices WHERE `MODELID`='BRIDGE' ORDER BY ID");
        $total = count($bridge);
        for ($i = 0; $i < $total; $i++) {


            $hue = new \AlphaHue\AlphaHue($bridge[$i]["IP"], $bridge[$i]["PASSWORD"]);
 ...
 huedevices: BRIDGEUUID varchar(50) NOT NULL DEFAULT ''
